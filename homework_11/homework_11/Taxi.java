...
public class Taxi{
	...
	public Taxi(int id, Findpath gps){
		/*@REQUIRES: (\all int id;1 <= id <= 100)
		 *@MODIFIES:random
		 *			id
		 *			state
		 *			last_state
	...
	public void refreshState(){
		...
		if (state.equals("waiting")||state.equals("stop"))
			watch -= 100;
		TaxiSystem.gui.SetTaxiStatus(this.id-1, new Point(point.getRow(), point.getCol()), 
				TaxiSystem.stateToint.get(getState()));
	}
...
//				应该选择随机最小流量
			...
			int light = guigv.lightmap[point.getRow()][point.getCol()];
			switch (light) {
			case 1:
			//north sorth red
				if ( (lastpoint.getCol() == next_point.getCol()&&lastpoint.getRow() != next_point.getRow())
			...
			case 2:
			//west east red
				if ( (lastpoint.getRow() == next_point.getRow()&&lastpoint.getCol() != next_point.getCol())
				  ||(lastpoint.getRow()+1 == next_point.getRow()&&lastpoint.getCol()+1 == next_point.getCol())
				  ||(lastpoint.getRow()-1 == next_point.getRow()&&lastpoint.getCol()-1 == next_point.getCol())){
					this.clock_count = -1*Light.getTimeleft();
//					System.out.println("stop here"+id+" loc:"+point);
				}
				break;
			default:
				break;
	...
	public boolean setNextmyPoint(myPoint temp){
		 ...
		 *				then return true
		 */
		if (!gps.isNeighbours(temp, point)){
			System.out.println(point+" "+temp+gps.getmyPointNeighbours(point).contains(temp));
			setRoute(gps.search(point, current_dst));
