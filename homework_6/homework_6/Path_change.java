...
public class Path_change implements Runnable{

	private LinkedList<Integer> taskNO;
	private Snapshot snap;
	
	
	public Path_change(Snapshot snap){
...
//			此处处理删除任务和结束某个监视器
		...
		for (TreeNode t:direct_parent.getChildList()){//新快照中父目录的子目录是不是有符合条件的文件
			...
			if (t.getName().equals(last_node.getName()) && t.getLast_time() == last_node.getLast_time() && t.getSize() == last_node.getSize()){
