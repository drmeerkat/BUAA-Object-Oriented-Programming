...
public class Readin implements Runnable{
		...
		try {
            ...
            if(file.isFile() && file.exists()){
                ...
                while((line = bufferedReader.readLine()) != null){
                	...
                	if (i != map_size || line_num == map_size-1) 
                		break;
                }
                read.close();
                if (line_num != map_size - 1){
	...
	public static boolean cr_request(String line){
		...
		if (match_request.matches()) {
			...
			if (!(Integer.parseInt(match_request.group(1)) == Integer.parseInt(match_request.group(3)) 
					&& Integer.parseInt(match_request.group(2)) == Integer.parseInt(match_request.group(4)) ) )
				return true;
		}
