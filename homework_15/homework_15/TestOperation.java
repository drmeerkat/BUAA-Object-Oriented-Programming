...
//		将时间直接截断到100ms 查询100ms为单位的状态
		...
		if (testSys.getClock() == temp){
			if (NO <= 0 || NO > 100){
				System.out.println("Invalid check taxi request");
				testlog.add("-----------------------------------------");
				testlog.add(time+": "+mission_num+" Invalid check taxi request");
				return;
			}
			mission_num++;
			testlog.add("-----------------------------------------");
	...
	public ListIterator<String> getIterator(int NO){
		/*@REQUIRES: (\all int NO;0 < NO <= 100)
		 *@MODIFIES: None
		 *@EFFECTS: if (is new taxi)
		 *				返回指定出租车的记录字符串迭代器
		 *			else
		 *				返回空迭代器
		 */
		LinkedList<String> list = new LinkedList<String>();
		if (testSys.getTaxi(NO).isnew){
			return ((PinkTaxi)testSys.getTaxi(NO)).iterator();
		}
		else
